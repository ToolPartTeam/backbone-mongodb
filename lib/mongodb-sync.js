  ...
  findOne: function(callback) {
    ...
    this._withCollection(function(err, collection) {
      ...
      collection.findOne({ _id: new ObjectID(model.id) }, function(err, dbModel) {
        if (err) callback(err);
        else if (!dbModel) callback(findOneError);
...
_.extend(Backbone.Model.prototype, Mongo.prototype, {
  ...
  _create_embedded: function(){
    ...
    _.each(this.embedded, function(embedAs, attribute){
      var self = this,
          nestwith,
      ...
      updateFromCollection = function(attribute){
        ...
        nestwith.find({_id: {$in: _.map( self.get(attribute), function(id){ return new ObjectID(id); })}}, 
          function(err, collection) {
            if(err) throw err;
            self[attribute] = nestwith.reset(collection, {silent: true});
...
_.extend(Backbone.Collection.prototype, Mongo.prototype, {
    ...
    find: function(args, qoptions, options) {
      if(!options) {
        options = qoptions;
        qoptions = {};
      }
      ...
      this._withCollection(function(err, collection) {
        if (err) { return options.error(err); }
          ...
          collection.find(args, qoptions).toArray(function(err, results) {
    ...
    // The callback is constructed from options.error and options.success
    //
    // @params args: the query arguments for findOne
    // @params qoptions (optional): additional arguments for query
    // @param options: either a callback or a bacbone callback json object
    // @returns: Collection.Model instance or null if the model does not validate
    findOne: function(args, qoptions, options) {
      if(!options) {
        options = qoptions;
        qoptions = {};
      }
      ...
      this._withCollection(function(err, collection) {
        ...
        var findOneError = new Error('Could not find anything');
        collection.findOne(args, qoptions, function(err, dbModel) {
          ...
          else if (!dbModel) options.error(findOneError);
          else {
            tr = new events.EventEmitter();
            tr.once('findOne:loaded', function(err, model){
              if(err) options.error(err);
