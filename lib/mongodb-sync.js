      ...
      if(_.isArray(this.get(attribute))) {
        ...
        if(_.isEmpty(this.get(attribute))) {
          ...
          updateFromCollection = function(attribute){
            this[attribute] = new nestAs(this.get(attribute), {container: this});
            ...
            this[attribute].bind('remove', function(model){
              var data = {};
              data[attribute] = _.without(this.get(attribute), model.id);
              this.set(data);
            });
          };
          updateFromCollection(attribute);
        ...
        if(_.isEmpty(self.get(attribute))) {
          ...
          // we have json
          self[attribute] = new nestAs(self.get(attribute), {container: self});
          self[attribute].bind('change', function(model){
            var data = {};
            data[attribute] = model.attributes;
            self.set(data);
          });
    ...
    if(_.isArray(this.get(attribute))) {
      ...
      if(_.isEmpty(self.get(attribute))) {
        ...
        updateFromCollection = function(attribute){
          ...
          nestwith.findOne({_id: new ObjectID(self.get(attribute))},
            function(err, value){
